...
@Client.on_message(filters.command(["sc"]) | filters.regex(r"^\$sc(\s|$)"))
async def handle_stripe_worker_charge(client, message):
    """
    Handle /sc command for Stripe Worker gate (mohsinop worker API).
    Usage: /sc cc|mm|yy|cvv
    """
    try:
...
<b>Example:</b> <code>/sc 4027667702076131|01|2026|505</code>""",
                reply_to_message_id=message.id,
                parse_mode=ParseMode.HTML
            )
