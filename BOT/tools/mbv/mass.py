...
import asyncio
from BOT.tools.mbv.api import async_check_mbv
...
@Client.on_message(filters.command("mmbv") | filters.regex(r"^\$mmbv(\s|$)"))
async def handle_mmbv_command(client, message):
    """Handle mass MBV check command: $mmbv with multiple cards"""
    ...
    if user_id in user_locks:
        return await message.reply(
            "<pre>⚠️ Wait!</pre>\n"
            "<b>Your previous</b> <code>$mmbv</code> <b>request is still processing.</b>\n"
...
<b>You have to register first using</b> <code>/register</code> <b>command.</b>""",
        ...
        if not target_text:
            return await message.reply(
                "❌ <b>Send cards!</b>\n1 per line:\n<code>5425233430109903|04|2026|168</code>",
                reply_to_message_id=message.id
            )
...
""",
        ...
        for idx, fullcc in enumerate(all_cards, start=1):
            ...
            result = await async_check_mbv(card, mes, ano, cvv)
            ...
            # Categorize result
            if status == "approved":
                status_flag = "MBV Passed ✅"
                approved_count += 1
            elif status == "declined":
                status_flag = "MBV Failed ❌"
                declined_count += 1
            else:
                status_flag = "Error ⚠️"
                error_count += 1
...
""",
        ...
        completion_message = f"""<pre>✦ MBV Check Completed</pre>
━━━━━━━━━━━━━━━
🟢 <b>Total CC</b>     : <code>{total_cc}</code>
💬 <b>Progress</b>    : <code>{processed_count}/{total_cc}</code>
✅ <b>MBV Passed</b>  : <code>{approved_count}</code>
❌ <b>MBV Failed</b>  : <code>{declined_count}</code>
...
━━━━━━━━━━━━━━━"""
        ...
        print(f"Error in $mmbv command: {str(e)}")
        try:
            await message.reply(
