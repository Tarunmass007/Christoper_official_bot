...
@Client.on_message(filters.command("mbv") | filters.regex(r"^\$mbv(\s|$)"))
async def handle_mbv_command(client, message):
    """Handle single MBV check command: $mbv cc|mes|ano|cvv"""
    ...
    if user_id in user_locks:
        return await message.reply(
            "<pre>⚠️ Wait!</pre>\n"
            "<b>Your previous</b> <code>$mbv</code> <b>request is still processing.</b>\n"
...
<b>You have to register first using</b> <code>/register</code> <b>command.</b>""",
        ...
        if not target_text:
            return await message.reply(
                "❌ <b>Send card in format:</b>\n<code>$mbv cc|mes|ano|cvv</code>\n\n"
                "<b>Example:</b> <code>$mbv 5425233430109903|04|2026|168</code>",
                reply_to_message_id=message.id
            )
        ...
        if not card_data:
            return await message.reply(
                ...
                "<b>Use:</b> <code>$mbv cc|mes|ano|cvv</code>",
                reply_to_message_id=message.id
            )
...
━━━━━━━━━━━━━━━""",
        ...
        final_message = f"""<pre>━━━ MBV Checker ━━━</pre>
<b>Card:</b> <code>{fullcc}</code>
...
<b>Dev:</b> <a href="https://t.me/Chr1shtopher">Chr1shtopher</a> <code>{current_time}</code>"""
        ...
        print(f"Error in $mbv command: {str(e)}")
        await message.reply(
