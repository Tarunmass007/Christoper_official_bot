...
# Note: We are using the same user_lock for redeems to prevent file access collisions 
# if we decide to split locks later, we can, but a global lock is safer given python's file IO.
# Actually, let's make a separate redeem_lock for the redeem file specifically.
...
async def redeem_code(client, message):
    ...
    # Acquire BOTH locks because we touch both files
    # Order matters to prevent deadlocks: User Lock -> Redeem Lock
